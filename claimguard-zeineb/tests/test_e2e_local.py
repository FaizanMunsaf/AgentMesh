"""Simulates the full pipeline as direct Python calls — no Band connection needed."""
import json
import pytest
from claimguard.shared.schemas import ClaimRecord, ClearanceRequest
...
SAMPLE_CLAIM = ClaimRecord(
    ...
    accident_description="Rear-ended at a red light by an uninsured driver, vehicle has front bumper damage",
    medical_details="Patient experienced mild whiplash, saw doctor twice",
    police_report_number="APD-2026-0042",
    other_party_info="John Smith, uninsured, license TX-ABC-1234",
...
def test_policy_lookup_confirms_active():
    result = json.loads(lookup_policy.invoke({"policy_number": "NVC-10001"}))
    assert result["active"] is True


def test_assessment_clearance_removes_sensitive_fields():
...
def test_payout_clearance_keeps_only_payment_fields():
    filtered, decisions = apply_policy(SAMPLE_CLAIM, "payout")
    ...
    assert filtered.incident_date is None


def test_decisions_log_written(tmp_path, monkeypatch):
    import claimguard.agents.firewall.engine as engine_module
    monkeypatch.setattr(engine_module, "DECISIONS_LOG", tmp_path / "decisions.jsonl")
    _, decisions = apply_policy(SAMPLE_CLAIM, "assessment")
